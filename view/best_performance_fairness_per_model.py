...
label_discrimination = "disparate_impact" # "average_predictive_value_difference" #"average_odds_difference" #"equal_opportunity_difference"# 
no_discrimination_line = 0.0 #1.00
...
# Seleciona pontos de melhor fairness e melhor performance
best_points = []
for model_name, group in df.groupby("model"):
    ...
    # Melhor performance (maior f1-score)
    idx_best_perf = group[label_performance].idxmax()
    row_perf = group.loc[idx_best_perf].copy()
    row_perf["best_type"] = "best performance"
    row_perf["legend_label"] = pipeline_description(row_perf)
...
for _, row in best_df.iterrows():
    ...
    if row["best_type"] == "best performance":
        facecolor = "red"
...
plt.title(f"{label_performance} vs {label_discrimination}  [best performance (red) e best fairness do modelo (branco)]")
