...
def gera_configs(params_path):
    ...
    if "model" in params_yaml:
        ...
        if isinstance(params_yaml["model"], list):
            ...
            # Caso params_path esteja no formato repetido (sem lista explícita)
    ...
    for stage in ["pre", "in", "post"]:
        if stage in params_yaml.get("mitigation", {}):
            ...
            print(f"{stage} methods:", mitigations_cfg[stage])  # Adicionando este print
        ...
        for pre in mitigations_cfg["pre"]:
            for in_ in mitigations_cfg["in"]:
                for post in mitigations_cfg["post"]:
                    ...
                    # Nome do arquivo
                    filename = f"{dataset_cfg['name']}__{model['name']}__pre-{pre['name']}__in-{in_['name']}__post-{post['name']}.yaml"
